...
| --- | --- | --- | --- |
| NT-1 | `adminEntry` virtual-module wiring nemusí fungovat out-of-the-box pro workspace balíček v této verzi (nikdy jsme to netestovali živě) | Blokuje celou práci, pokud se zjistí pozdě | NATIVE_PLAN N0 — nejmenší možný `pages: { "/reservations": () => <div>hello</div> }` smoke test hned na začátku, před psaním zbytku |
| NT-2 | Auth/session chování nepublic native rout neověřeno | Admin routy nemusí být chráněné, nebo naopak nedostupné z React stránky (CORS/cookies) | NATIVE_PLAN N0 — ověřit jednu no-op routu z prohlížeče přes `apiFetch` |
| NT-3 | GET/`input:` bug (PLAN fáze 3) se může, ale nemusí opakovat u native dispatcheru | Špatně navržené GET-only admin routy by tiše padaly | Návrh v §4 obchází riziko úplně (všechny admin routy POST) |
| NT-4 | Duplicitní deklarace `id`/`version`/`capabilities`/`storage` mezi descriptorem a `definePlugin()` | Rozjetí při update jen na jedné straně | NATIVE_PLAN N1 explicitní diff-check kroku |
